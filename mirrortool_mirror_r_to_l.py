...
class MIRRORTOOL_OT_Mirror_R_To_L_80709(bpy.types.Operator):
    bl_idname = "mirrortool.mirror_r_to_l_80709"
    bl_label = "Mirror R to L"
    bl_description = "Mirror controllers from right to left side with adjustable strength"
    ...
    def invoke(self, context, event):
        ...
        for bone in bones:
            # Apply only from Right to Left
            if bone.name.endswith(".L") or bone.name.endswith("_L") or bone.name.endswith(".l"):
                continue
...
def register():
    bpy.utils.register_class(MIRRORTOOL_OT_Mirror_R_To_L_80709)


def unregister():
    bpy.utils.unregister_class(MIRRORTOOL_OT_Mirror_R_To_L_80709)
